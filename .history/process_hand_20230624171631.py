...
class DataSaver:
    ...
    def writeData(self, handworldLandmarks_points, label:int):
        for i in range(0, 3):
            print("comeon!:", handworldLandmarks_points[0][i])
    ...
    def readytosave(self, key, handworldLandmarks_points, flip=False, curPred=0):
        ...
        if handworldLandmarks_points is not None:
            for i in range(0, 3):
                print("ohman!:", handworldLandmarks_points[0][i])

        print("状态:", self.start_save, "当前预测:", curPred, "编号:", self.save_label, "次数:", self.save_num)
        if self.start_save:
            self.writeData(handworldLandmarks_points, self.save_label)
            self.save_num += 1
...
class mediaPipeHand:
    ...
    def get_landmarks(self, color_image, draw_fingers=True):
        ...
        if results.multi_hand_world_landmarks and results.multi_hand_landmarks:
            ...
            for i in range(0, 3):
                print("fuckyou!:", handworldLandmarks_points[0][i])
