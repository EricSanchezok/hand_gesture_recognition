...
def main():
    ...
    while True:
        ...
        pred_val, pred_index = pg.get_pred(points, angle_list, IsPrint=True)
        ...
        key = cv2.waitKey(1)
        if saveMode:
            data_saver.readytosave(key, results, flip=False)
            data_saver.readytosave(key, flip_results, flip=True)
