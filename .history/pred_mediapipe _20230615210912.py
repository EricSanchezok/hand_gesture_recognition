...
def normalize(tensor):
    ...
    maxvalue = tensor.max(dim=1, keepdim=True)
    minvalue = tensor.min(dim=1, keepdim=True)

    # 标准化张量
    normalized_tensor = (tensor - mean) / (maxvalue - minvalue)
