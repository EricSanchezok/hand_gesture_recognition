...
class mediaPipeHand:
    ...
    def get_world_points(self, color_image, drawPoints=True):
        ...
        show_color_image = color_image.copy()

        results = self.hands.process(cv2.cvtColor(show_color_image, cv2.COLOR_BGR2RGB))

        if results.multi_hand_landmarks and drawPoints:
            for handLms in results.multi_hand_landmarks:
                self.mpDraw.draw_landmarks(show_color_image, handLms, self.mpHands.HAND_CONNECTIONS, self.handLmsStyle, self.handConStyle)
        ...
        return results, points, show_color_image
...
try:
    while True:
        ...
        results, points, show_color_image = hand_detector.get_world_points(color_image, drawPoints=True)
