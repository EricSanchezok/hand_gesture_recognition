...
class plot_3D:
    def __init__(self, title, figsize=(8, 6), fontsize=14, color='red', xlim =[0.0, 1.0], ylim=[-0.5, 0.5], zlim=[-0.5, 0.5], numshow=100) -> None:
        ...
        if if_play:
            ...
            self.sphere_x_list, self.sphere_y_list, self.sphere_z_list = self.get_sphere(self.x_t, self.y_t, self.z_t, self.r)
    ...
    def draw_3Dvalue(self, x_value, y_value, z_value):
        ...
        if if_play:
            ...
            if self.cal_dis(x_value, y_value, z_value) < 0.2:
                self.x_t, self.y_t, self.z_t = self.get_random_target()
                self.numshow += 10
    ...
    def get_random_sphere(self, x, y, z, r):
        # 在x, y, z点附近的随机生成100个距离为r的点
        x_list = []
        y_list = []
        z_list = []
        for i in range(100):
            x_list.append(np.random.uniform(x - r, x + r))
            y_list.append(np.random.uniform(y - r, y + r))
            z_list.append(np.random.uniform(z - r, z + r))
