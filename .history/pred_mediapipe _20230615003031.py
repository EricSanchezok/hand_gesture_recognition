...
hands = mpHands.Hands(static_image_mode=False, max_num_hands=1, model_complexity=1, min_detection_confidence=0.8, min_tracking_confidence=0.5)
mpDraw = mp.solutions.drawing_utils
handLmsStyle = mpDraw.DrawingSpec(color=(0,0,255), thickness=5)
handConStyle = mpDraw.DrawingSpec(color=(0,255,0), thickness=5)

#读取训练好xgboost模型
bst = xgb.Booster(model_file='xgboost_model.model')
...
try:
    while True:
        ...
        if results.multi_hand_landmarks:
            ...
            #标准化
            points = (points - points.min()) / points.std()
            #print("shape: ", points.shape)
            pred = bst.predict(xgb.DMatrix(points))
            print("pred: ", pred)
